    ...
    >
      ...
      <div className="network-incident-grid">
        <fieldset className={`network-incident-card ${evaluations["longest-prefix"]?.correct ? "is-correct" : evaluations["longest-prefix"] ? "is-incorrect" : ""}`}>
          ...
          <label><span>gateway</span><select aria-label={t("route 사건 gateway", "Route incident gateway")} value={drafts["longest-prefix"].routeGateway ?? ""} onChange={(event) => setField("longest-prefix", "routeGateway", event.target.value)}><option value="">—</option><option value="10.0.0.1">10.0.0.1</option><option value="10.0.0.253">10.0.0.253</option><option value="10.0.0.252">10.0.0.252</option></select></label>
          <label><span>interface</span><select aria-label={t("route 사건 egress interface", "Route incident egress interface")} value={drafts["longest-prefix"].routeInterfaceId ?? ""} onChange={(event) => setField("longest-prefix", "routeInterfaceId", event.target.value)}><option value="">—</option><option value="eth0">eth0</option><option value="lo">lo</option></select></label>
        ...
        <fieldset className={`network-incident-card ${evaluations["next-hop-frame"]?.correct ? "is-correct" : evaluations["next-hop-frame"] ? "is-incorrect" : ""}`}>
          ...
          <label><span>next hop</span><select aria-label={t("frame 사건 next hop", "Frame incident next hop")} value={drafts["next-hop-frame"].nextHop ?? ""} onChange={(event) => setField("next-hop-frame", "nextHop", event.target.value)}><option value="">—</option><option value="192.0.2.1">192.0.2.1</option><option value="203.0.113.20">203.0.113.20</option></select></label>
          <label><span>Ethernet dst</span><select aria-label={t("frame 사건 Ethernet 목적지", "Frame incident Ethernet destination")} value={drafts["next-hop-frame"].ethernetDestination ?? ""} onChange={(event) => setField("next-hop-frame", "ethernetDestination", event.target.value)}><option value="">—</option><option value="02:00:00:00:00:21">02:00:00:00:00:21</option><option value="02:00:00:00:00:20">02:00:00:00:00:20</option></select></label>
          <label><span>IPv4 dst</span><select aria-label={t("frame 사건 IPv4 목적지", "Frame incident IPv4 destination")} value={drafts["next-hop-frame"].ipDestination ?? ""} onChange={(event) => setField("next-hop-frame", "ipDestination", event.target.value)}><option value="">—</option><option value="203.0.113.20">203.0.113.20</option><option value="192.0.2.1">192.0.2.1</option></select></label>
        ...
        <fieldset className={`network-incident-card ${evaluations["listener-delivery"]?.correct ? "is-correct" : evaluations["listener-delivery"] ? "is-incorrect" : ""}`}>
          ...
          <label><span>{t("IP packet 전달", "Network delivered")}</span><select aria-label={t("listener 사건 network delivery", "Listener incident network delivery")} value={drafts["listener-delivery"].networkDelivered ?? ""} onChange={(event) => setField("listener-delivery", "networkDelivered", event.target.value)}><option value="">—</option><option value="true">{t("예", "Yes")}</option><option value="false">{t("아니요", "No")}</option></select></label>
          <label><span>{t("listener 일치", "Listener matched")}</span><select aria-label={t("listener 사건 listener match", "Listener incident listener match")} value={drafts["listener-delivery"].listenerMatched ?? ""} onChange={(event) => setField("listener-delivery", "listenerMatched", event.target.value)}><option value="">—</option><option value="true">{t("예", "Yes")}</option><option value="false">{t("아니요", "No")}</option></select></label>
          <label><span>TCP response</span><select aria-label={t("listener 사건 TCP 응답", "Listener incident TCP response")} value={drafts["listener-delivery"].listenerResponse ?? ""} onChange={(event) => setField("listener-delivery", "listenerResponse", event.target.value)}><option value="">—</option><option value="syn-ack">SYN-ACK</option><option value="rst">RST</option><option value="none">none</option></select></label>
          <label><span>{t("application byte 인수", "Application received bytes")}</span><select aria-label={t("listener 사건 application delivery", "Listener incident application delivery")} value={drafts["listener-delivery"].applicationDelivered ?? ""} onChange={(event) => setField("listener-delivery", "applicationDelivered", event.target.value)}><option value="">—</option><option value="true">{t("예", "Yes")}</option><option value="false">{t("아니요", "No")}</option></select></label>
