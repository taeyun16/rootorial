...
로컬에서 아직 공개되지 않은 구현 완료 커리큘럼과 챕터까지 검수할 때는 전용
읽기 전용 개발 모드를 사용합니다.
...
`/admin/preview/curricula/`에서 구현 및 계획 상태를 구분한 전체 감사 인덱스를
열고, 미리보기 준비가 끝난 커리큘럼과 챕터를 탐색할 수 있습니다.
이 모드는 Vite의 명시적 `content-preview` 개발 모드와 `localhost`/loopback 요청이 모두 맞을 때만
활성화되며 관리자 변경 API, 토론, 분석, 진도 저장 권한은 열지 않습니다.
