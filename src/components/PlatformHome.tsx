...
const copy = {
  ko: {
    ...
    mapCaption: "FIRST PRINCIPLES · INDEPENDENT LEARNING PATHS",
    ...
    roadmapSummary: "Rootorial의 다음 학습 여정입니다. 공개 전까지 각 커리큘럼의 기초 구조와 실습을 차례로 다듬습니다.",
  ...
  en: {
    ...
    roadmapSummary: "These are the next Rootorial learning journeys. Before launch, we shape each path's foundations and hands-on practice in sequence.",
    philosophy: "ROOTORIAL METHOD",
...
export function PlatformHome({
  ...
  const continueCurriculum = availableCurricula
    .map((item) => ({
      item,
      completedCount: completed.filter((id) =>
        id.startsWith(`${item.curriculum.slug}/`),
      ).length,
    }))
    .sort((left, right) => right.completedCount - left.completedCount)
    .find(({ completedCount }) => completedCount > 0)?.item ?? featured;
