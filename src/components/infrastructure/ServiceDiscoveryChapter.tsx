      ...
      <div className="article-layout">
        ...
        <article className="lesson-article">
          ...
          <section className="article-section" id="ttl">
            ...
            <p>{t(
              "cached_at=100, TTL=60이면 record는 `now < 160` 동안 fresh입니다. 따라서 t=159에는 기존 10.40.0.10을 쓰고 t=160부터 authority의 10.40.0.20을 다시 얻습니다. migration은 이 최대 cache lifetime을 고려해 기존 VIP를 적어도 t=160까지 유지해야 합니다. TTL을 짧게 하는 것과 health check는 서로 대체할 수 없습니다.",
              "With cached_at=100 and TTL=60, the record is fresh while `now < 160`. The resolver therefore uses the old 10.40.0.10 at t=159 and refreshes to the authority's 10.40.0.20 at t=160. A migration must keep the old VIP through that maximum cache lifetime. A short TTL and a health check do not replace each other.",
            )}</p>
            <div className="service-invariant-table" role="table" aria-label={t("DNS TTL invariant", "DNS TTL invariants")}><div role="row"><strong role="cell">FRESH</strong><span role="cell">now &lt; cached_at + TTL</span></div><div role="row"><strong role="cell">EXPIRED</strong><span role="cell">now ≥ cached_at + TTL → authority refresh</span></div><div role="row"><strong role="cell">ROLLOUT</strong><span role="cell">old VIP lifetime ≥ maximum valid cache lifetime</span></div></div>
          </section>
