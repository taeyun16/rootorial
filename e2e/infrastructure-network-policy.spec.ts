...
async function configureWorkingPolicy(lab: Locator, locale: "ko" | "en") {
  ...
  await lab.getByLabel(isKo ? "base chain hook" : "Base chain hook").selectOption(activeMode);
  await lab.getByLabel(isKo ? "base chain policy" : "Base chain policy").selectOption("drop");
  await lab.getByLabel("ct state established,related accept").check();
  await lab.getByLabel(isKo ? "required allow scope" : "Required allow scope").selectOption("exact");
  await lab.getByLabel(isKo ? "terminal rule order" : "Terminal rule order").selectOption("stateful-specific-deny");
  await lab.getByLabel(isKo ? "policy 실행 결과 예측" : "Predict the policy result").selectOption("intended-only");
}
...
function policyOverflow(page: TestPage) {
  return page.locator(
    ".network-policy-chapter-shell, .network-policy-preflight, .network-policy-hook-grid, .network-policy-lab, .network-policy-control-grid, .network-policy-visualization, .network-policy-map, .network-policy-boundary, .network-policy-trace-grid, .network-policy-chain, .network-policy-probes, .network-policy-command-evidence, .network-policy-incident-lab, .network-policy-incident-grid, .network-policy-incident-card, .network-completion-checklist",
...
test("completes FORWARD, INPUT, incidents, and concepts in the Korean draft preview", async ({ page }) => {
  ...
  const visual = lab.getByTestId("network-policy-visualization");
  await expect(visual.getByRole("img", { name: /FORWARD와 INPUT firewall policy 지도/ })).toBeVisible();
  ...
  await lab.getByLabel("required allow scope").selectOption("any-source");
  await expect(visual).toHaveAttribute("data-policy-state", "overbroad");
  await expect(visual).toHaveAttribute("data-grade-state", "not-run");
  await lab.getByLabel("policy 실행 결과 예측").selectOption("intended-only");
  await lab.getByRole("button", { name: "packet probe suite 실행" }).click();
  await expect(visual).toHaveAttribute("data-grade-state", "failed");
  await lab.getByLabel("required allow scope").selectOption("exact");
  await lab.getByLabel("policy 실행 결과 예측").selectOption("intended-only");
  await lab.getByRole("button", { name: "packet probe suite 실행" }).click();
  await expect(visual).toHaveAttribute("data-grade-state", "passed");
...
test("keeps the English draft keyboard-usable at 390px without overflow or untranslated text", async ({ page }) => {
  ...
  const lab = page.locator(".network-policy-lab");
  const visual = lab.getByTestId("network-policy-visualization");
  await expect(visual.getByRole("img", { name: /FORWARD and INPUT firewall policy map/ })).toBeVisible();
  ...
  await expect(visual).toHaveAttribute("data-grade-state", "not-run");
  await expect(lab.getByLabel("Predict the policy result")).toHaveValue("");

  const firstIncident = page.locator(".network-policy-incident-card").first();
  await firstIncident.getByRole("combobox").selectOption("move-service-rule-to-forward");
