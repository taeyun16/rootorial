    ...
    >
      ...
      <fieldset className="network-prediction-fieldset">
        ...
        <div className="network-prediction-grid">
          <ChoiceField label={t("fd · connect · send 경계", "Fd, connect, and send boundaries")} value={answers.boundaryPath} onValueChange={(value) => updateAnswer("boundaryPath", value)} options={[{ value: "", label: "—" }, { value: "fd-handshake-queue", label: t("fd는 local 참조 · connect는 handshake · send는 local queue", "Fd is local · connect waits for handshake · send enqueues locally") }, { value: "wire-dns-delivered", label: t("fd가 wire로 이동 · 이름 해석 뒤 연결 · send는 remote 처리", "Fd travels on wire · name resolution connects · send proves remote processing") }]} />
          <ChoiceField label={t("same-link 10.0.0.42", "Same-link 10.0.0.42")} value={answers.localRoute} onValueChange={(value) => updateAnswer("localRoute", value)} options={[{ value: "", label: "—" }, { value: "direct-24-peer", label: "/24 direct · next hop 10.0.0.42 · peer MAC" }, { value: "default-gateway", label: "/0 via 10.0.0.1 · gateway MAC" }]} />
          <ChoiceField label={t("remote route · frame", "Remote route and frame")} value={answers.remoteRoute} onValueChange={(value) => updateAnswer("remoteRoute", value)} options={[{ value: "", label: "—" }, { value: "default-gateway-preserve-ip", label: "/0 via 10.0.0.1 · gateway MAC · IP dst 203.0.113.20" }, { value: "direct-server-rewrite-ip", label: "/24 direct · server MAC · IP dst 10.0.0.1" }]} />
          <ChoiceField label={t("MTU · MSS · sequence", "MTU, MSS, and sequence")} value={answers.segmentation} onValueChange={(value) => updateAnswer("segmentation", value)} options={[{ value: "", label: "—" }, { value: "mss-1460-syn-consumes-one", label: "MSS 1460 · payload 1460/1460/80 · seq 1001/2461/3921" }, { value: "mtu-1500-syn-free", label: "MSS 1486 · payload 1500/1500 · seq 1000/2460/3920" }]} />
          <ChoiceField label={t("gap · RTO · cumulative ACK", "Gap, RTO, and cumulative ACK")} value={answers.lossRecovery} onValueChange={(value) => updateAnswer("lossRecovery", value)} options={[{ value: "", label: "—" }, { value: "gap-rto-cumulative", label: "ACK 2461 → 2461 · RTO seq 2461 · final ACK 4001" }, { value: "skip-gap-new-sequence", label: "ACK 2461 → 4001 · new seq 4001 · final ACK 3921" }]} />
        </div>
      ...
      <div className="network-action-grid" role="group" aria-label={t("네트워크 상태 전이", "Network state transitions")}>
        ...
        <ChoiceField className="network-disposition-control" label={t("다음 segment의 wire 결과", "Next segment wire result")} value={segmentDisposition} disabled={machine.segments.length !== 3 || machine.segments.every((segment) => segment.transmissions > 0)} onValueChange={setSegmentDisposition} options={[{ value: "", label: "—" }, { value: "deliver", label: t("전달", "Deliver") }, { value: "drop", label: t("유실", "Drop") }]} />
