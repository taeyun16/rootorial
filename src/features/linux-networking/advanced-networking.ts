...
export const advancedLinuxNetworkingConfigs: Record<AdvancedLinuxNetworkingSlug, AdvancedChapterConfig> = {
  ...
  "dns-and-service-reachability": {
    ...
    foundation: { title: t("resolver는 이름을 주소로 바꾸지만 서비스 상태를 보장하지 않습니다", "A resolver turns a name into an address but does not guarantee service health"), body: t("getent는 application이 사용하는 이름 해석 경로를 보여 주고 dig는 DNS 응답과 TTL을 자세히 보여 줍니다. 일반적인 cache에서 TTL은 resolver가 출처를 다시 확인해야 하는 시점이며, 권한 DNS 갱신이 실패할 때만 명시적인 복원력 정책이 만료 데이터를 serve-stale로 제공할 수 있습니다. 어느 출력도 listener를 증명하지는 않습니다.", "getent shows the name-resolution path used by applications, while dig exposes DNS answers and TTLs. In ordinary caching, TTL marks when the resolver must reconsult the source; only an explicit serve-stale resiliency policy may use expired data when authoritative refresh fails. Neither output proves that a process listens on the target port.") },
    ...
    figure: {
      ...
      phases: [
        ...
        { id: "response", label: t("응답 검증", "Verify response"), command: "curl -sS -w '\\nHTTP %{http_code}\\n' http://api.lab.test:8080/health", output: ['{"status":"ok"}', "HTTP 200"], activeNodes: ["client", "endpoint", "service"], activeEdges: ["cache-endpoint", "endpoint-service"], decision: t("기대한 application 본문과 HTTP 상태를 함께 검증했습니다.", "The expected application body and HTTP status are verified together."), facts: [fact("HTTP", "HTTP", "200"), fact("health", "HEALTH", "status=ok")] },
      ],
    },
    incidents: [
    ...
    questions: [
      ...
      { id: "ttl-role", prompt: t("record TTL의 역할은?", "What is a record TTL for?"), options: [option("cache-lifetime", "resolver가 출처를 다시 확인해야 하는 시점", "When the resolver must reconsult the source"), option("packet-hops", "IP packet의 hop 수", "IP packet hop count"), option("tcp-timeout", "TCP connect timeout", "TCP connect timeout")], correctAnswer: "cache-lifetime" },
  ...
  "diagnose-a-linux-network": {
    ...
    figure: {
      ...
      phases: [
        ...
        { id: "scope-response", label: t("응답 검증", "Response proof"), command: "curl -fsS -w '\\nHTTP %{http_code}\\n' http://api.lab.test:8080/health", output: ["{\"status\":\"ok\"}", "HTTP 200"], activeNodes: ["name", "response"], activeEdges: ["name-response"], decision: t("기대한 응답 본문과 HTTP 상태로 전체 경로를 검증했습니다.", "The expected response body and HTTP status verify the entire path."), facts: [fact("HTTP", "HTTP", "200"), fact("body", "BODY", "status=ok")] },
      ],
    },
    incidents: [
